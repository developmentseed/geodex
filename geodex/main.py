...
def parse_args(args):
    ...
    parser.add_argument('geojson', type=str,
                         help='Path to geojson file containing at least one boundary.')
    ...
    #pparser.add_argument('--output-format', '-o', default='{x} {y} {z}',
    #                     type=str, help='Output tile format. This is only valid for tile-formats `google` or `tms`')
...
def cli():
    ...
    logger.info('Start time: %s', st_dt)
    ...
    process_geojson(geojson_bounds, zoom, tile_format)
